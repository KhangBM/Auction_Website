...
const HomePage = () => {
  ...
  const fetchUpcomingAuctions = async (time = currentTime) => {
    try {
      const response = await axios.get(
        `https://e-auction-api.up.railway.app/v1/auction/list-client?status=1&fromBidTime=${time}`
      );
    ...
    <div className=" w-full mt-[60px]  grid grid-flow-col grid-cols-[15%_85%]">
      ...
      <div className=" m-4 flex flex-col gap-12">
        ...
        <div className="tren flex flex-col gap-2 ">
          ...
          <div className=" grid grid-flow-row grid-cols-[25%_25%_25%_25%] gap-2"></div>
        </div>
        <div className="tren flex flex-col gap-2 ">
          <div className=" flex flex-row justify-between">
            <div className="flex flex-row gap-2">
          ...
          <div className=" grid grid-flow-row grid-cols-[25%_25%_25%_25%] gap-2">
            {auctionList?.content?.slice(0, 8).map((auction, index) => (
              <div key={index}>
                <Item
                  img={auction.product.mainImage}
